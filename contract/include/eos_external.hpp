...
class eos_external: public External {
  private:
    ...
    TransferType determineTransferType(const uint256_t& senderAddress, const uint256_t& toAddressWord) {
      if (senderAddress == toAddressWord) return TransferType::TRANSFER_PARENT_CHILD_OUTGOING;
    ...
    emplace_t contractTransfer(const uint256_t& senderAddress, const uint256_t& toAddressWord, const uint256_t& value) {
      ...
      auto accountCodeItr = accountCodeIdx.find(targetAddress);
  ...
  public:
    ...
    uint256_t balance(const uint256_t& addressWord) {
      // TODO: always take subbalance first if it exists
    ...
    emplace_t transfer(const uint256_t& senderAddress, const uint256_t& toAddressWord, const uint256_t& value) {
      ...
      if (transferType == TransferType::TRANSFER_PARENT_CHILD_OUTGOING) return contractTransfer(senderAddress, toAddressWord, value);
