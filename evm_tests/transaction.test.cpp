...
TEST_CASE("Transaction call", "[transaction)]") {
  ...
  CHECK("" == Hex::bytesToHex(data));
  CHECK("1b" == Hex::bytesToHex(v));
...
TEST_CASE("Transaction digest", "[transaction)]") {
  ...
  CHECK(uint256_t(0x0) == Transaction::value(rlp));
  CHECK("0000000000000000" == Hex::bytesToHex(data));
  CHECK("01" == Hex::bytesToHex(v));
  CHECK("" == 
    Hex::bytesToHex(r)
  );
  CHECK("" == 
