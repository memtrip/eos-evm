...
import com.memtrip.evm.eos.evm.contracts.misc.SimpleAuctionContract
...
class SimpleAuctionTest {
    ...
    fun `The SimpleAuction contract is created`() {
        ...
        val createContract = simpleAuctionContract.createSimpleAuction(5, contractAccountIdentifier.toHexString()).blockingGet()
    ...
    fun `Bid on the simple auction`() {
        ...
        if (accountStateResult !is GetAccountState.Record.Multiple) fail("no state saved") else {
            ...
            assertEquals(accountStateResult.items[0].value, simpleAuctionContract.ownerAccountIdentifier.pad256().toHexString())
            // items[1] is (now + bidding_time) in seconds
    ...
    fun `AddressX bids on the simple auction, AddressY out bids AddressX, AddressX withdraws their bid`() {
        ...
        val createContract = simpleAuctionContract.createSimpleAuction(3, beneficiaryAccountIdentifier.toHexString()).blockingGet()
        assertEquals(202, createContract.statusCode)
        ...
        if (addressXBalanceAfterBid !is GetAccount.Record.Single) fail("Failed to check bid balance") else {
            assertEquals("0.9000 EVM", addressXBalanceAfterBid.item.balance.toString())
        }
        ...
        if (addressYBalanceAfterBid !is GetAccount.Record.Single) fail("Failed to check bid balance") else {
            assertEquals("0.8000 EVM", addressYBalanceAfterBid.item.balance.toString())
        }

        // and when
        val accountStateAfterBid2 = getAccountState.getAll(createContract.parentContractAddress32).blockingGet()
        ...
        if (accountStateAfterBid2 !is GetAccountState.Record.Multiple) fail("no state saved") else {
            ...
            assertEquals(accountStateAfterBid2.items[3].value, "00000000000000000000000000000000000000000000000002c68af0bb140000") // bid 2
            assertEquals(accountStateAfterBid2.items[4].value, "000000000000000000000000000000000000000000000000016345785d8a0000") // bid 1
        }
        ...
        auctionEndResponse.assertConsoleString(
            ...
            "${addressYAccountIdentifier.pad256().toHexString()}00000000000000000000000000000000000000000000000002c68af0bb140000"
        )
        ...
        if (beneficiaryBalanceAfterAuctionEnd !is GetAccount.Record.Single) fail("Failed to check beneficiary balance") else {
            assertEquals("1.2000 EVM", beneficiaryBalanceAfterAuctionEnd.item.balance.toString())
        }
        ...
        if (addressYAfterAuctionEnd !is GetAccount.Record.Single) fail("Failed to check addressY balance") else {
            assertEquals("0.8000 EVM", addressYAfterAuctionEnd.item.balance.toString())
        }
    }
}