...
import com.memtrip.evm.eos.evm.contracts.misc.BallotContract
...
class BallotTest {
    ...
    fun `Give an address the right to vote, and delegate that vote to the chair person`() {
        ...
        val giveRightToVoteResponse = ballotContract.giveRightToVote(senderAccountIdentifier.toHexString(), EvmSender(
            ...
            chairAccountName,
            chairPrivateKey,
            ballotContract.ownerAccountIdentifier.toHexString()
        )).blockingGet()
        ...
        // and when
        val delegateVoteResponse = ballotContract.delegate(ballotContract.ownerAccountIdentifier.toHexString(), EvmSender(
            1,
            senderEthAccount,
            senderAccountName,
            senderPrivateKey,
