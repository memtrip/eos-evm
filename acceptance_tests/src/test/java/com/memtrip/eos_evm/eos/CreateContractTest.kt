...
class CreateContractTest {
    ...
    fun `Create a contract via CREATE with a CODECOPY child contract that contains a CALL`() {
        ...
        if (result !is GetCode.Record.Multiple) fail("account_code record not found") else {
            ...
            assertNotEquals(result.items[0].address, result.items[1].address)
        }
    ...
    @Test
    fun `Create a contract with CREATE that contains two delegate calls`() {
        ...
        if (getCodeResult !is GetCode.Record.Multiple) fail("code record not found") else {
            ...
            assertNotEquals(getCodeResult.items[0].address, getCodeResult.items[1].address)
            assertNotEquals(getCodeResult.items[1].address, getCodeResult.items[2].address)
            assertNotEquals(getCodeResult.items[0].address, getCodeResult.items[2].address)
        }
