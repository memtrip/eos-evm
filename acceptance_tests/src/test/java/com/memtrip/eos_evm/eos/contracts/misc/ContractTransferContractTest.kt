...
import com.memtrip.eos_evm.eos.evm.contracts.misc.ContractTransferContract
...
class ContractTransferContractTest {
    ...
    fun `The ContractTransfer contract is created`() {
        ...
        val createContractResponse = faultTolerant {
            contract.createContract(EthAsset.milliether(300)).blockingGet()
        }
        ...
        if (getCodeResult !is GetCode.Record.Multiple) fail("code record not found") else {
            ...
            assertEquals("1.0", getCodeResult.items[2].nonce)
            assertEquals(getCodeResult.items[2].address, contract.accountIdentifier.pad256().toHexString())
        }
    }

    @Test
    fun `Call the transfer method`() {
        // given
        val (newAccountName, newAccountPrivateKey, newEthAccount) = setupTransactions.seedWithEvmBalance()
